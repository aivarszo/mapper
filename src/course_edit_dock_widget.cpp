 ...
 : QDockWidget(tr("Course edit"), parent), controller(controller)
{
    ...
    QStringList *ss=controller->getMap()->getcontrolpoints(rownum);
...
void courseEditDockWidget::set_cd_icon()
{
    ...
    controller->getMap()->setcontrolpointstext(cdn,rownum,qobject_cast<QToolButton*>(QObject::sender())->text().mid(1).toInt()*COURSE_ITEMS+6+(qobject_cast<QToolButton*>(QObject::sender())->text().mid(0,1).toStdString().c_str()[0]-'C'));
}
...
QToolButton* courseEditDockWidget::newToolButton(int k, const QIcon& icon, const QString& text)
{
    ...
    for (int i=0; i<controller->getMap()->getNumSymbols(); i++)
    {
        ...
        if (symstr->getNumberAsString()==controller->getMap()->getcontrolpoints(rownum)->at(k))
        {
...
void courseEditDockWidget::savecourse()
{
    ...
    controller->getMap()->setcontrolpoints(ss,rownum);
    cw->updateRow(rownum);
}
